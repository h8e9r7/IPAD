...
from modules.model import build_model, build_memorybank
...
from modules.loss.build import DELoss, PrototypeTripletRankingLoss, ValueCrossEntropy
...
def main(cfg):
    ...
    # inint memorybank
    mem = build_memorybank(cfg)
    mem2 = build_memorybank(cfg)
    ...
    # inint prototype loss
    proto_criterion = PrototypeTripletRankingLoss(cfg).cuda()
    # value classify
    vce_criterion = ValueCrossEntropy(cfg).cuda()
    ...
    logger.info(f'Total training time: {total_time_str}, Best mAP: {best_mAP}')
