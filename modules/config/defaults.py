from yacs.config import CfgNode as CN
import logging

# -----------------------------------------------------------------------------
# Config definition
# -----------------------------------------------------------------------------
_C = CN()

# Project name
_C.NAME = ""
# Checkpoint save dir
_C.SAVE_DIR = ""
# Device
_C.DEVICE = "cuda"

# Model Related
_C.MODEL = CN()
_C.MODEL.EMBED_SIZE = 1024
_C.MODEL.ATTRIBUTE = CN()
_C.MODEL.ATTRIBUTE.EMBED_SIZE = 512

_C.MODEL.GLOBAL = CN()
_C.MODEL.GLOBAL.BACKBONE = CN()
_C.MODEL.GLOBAL.BACKBONE.NAME = 'resnet50'
_C.MODEL.GLOBAL.BACKBONE.EMBED_SIZE = 1024
_C.MODEL.GLOBAL.ATTENTION = CN()
_C.MODEL.GLOBAL.ATTENTION.SPATIAL = CN()
_C.MODEL.GLOBAL.ATTENTION.SPATIAL.COMMON_EMBED_SIZE = 512
_C.MODEL.GLOBAL.ATTENTION.SPATIAL.REDUCTION_RATE = 2



_C.MODEL.TRANSFORMER = CN()
_C.MODEL.TRANSFORMER.ENABLE = False
_C.MODEL.TRANSFORMER.PATCH_SIZE = 16
_C.MODEL.TRANSFORMER.EMBED_SIZE = 768
_C.MODEL.TRANSFORMER.SPLIT ='non-overlap'
_C.MODEL.TRANSFORMER.NUM_LAYERS = 12
_C.MODEL.TRANSFORMER.SLIDE_STEP = 12
_C.MODEL.TRANSFORMER.DROPOUT_RATE = 0.1
_C.MODEL.TRANSFORMER.MLP_DIM = 3072
_C.MODEL.TRANSFORMER.NUM_HEADS = 12
_C.MODEL.TRANSFORMER.CROSS_NUM_HEADS = 12
_C.MODEL.TRANSFORMER.ATT_DROPOUT_RATE = 0.0
_C.MODEL.TRANSFORMER.REDUCTION_RATE = 2


_C.MODEL.SINGLE= CN()
_C.MODEL.SINGLE.ENABLE = True


# Input option
_C.INPUT = CN()
_C.INPUT.PIXEL_MEAN = [0.485, 0.456, 0.406]
_C.INPUT.PIXEL_STD = [0.229, 0.224, 0.225]
_C.INPUT.GLOBAL_SIZE = 224
_C.INPUT.LOCAL_SIZE= 224
_C.INPUT.THRESHOLD = 0.4

# Dataset option
_C.DATA = CN()
_C.DATA.NUM_WORKERS = 16
_C.DATA.NUM_TRIPLETS = 0  # 100000

_C.DATA.TRAIN_BATCHSIZE = 64 # 32 - 11G 64 - 24G
_C.DATA.TEST_BATCHSIZE = 64
_C.DATA.BASE_PATH = "../data"
_C.DATA.DATASET = ""
_C.DATA.NUM_ATTRIBUTES = -1

# Attributes, refer to specific dataset configuration file
_C.DATA.ATTRIBUTES = CN(new_allowed=True)
# modify when change dataset
_C.DATA.ATTRIBUTES.NUM = [6, 9, 8, 6, 5, 5, 10, 5]

_C.DATA.PATH_FILE = CN()
_C.DATA.PATH_FILE.TRAIN = ""
_C.DATA.PATH_FILE.VALID = ""
_C.DATA.PATH_FILE.TEST = ""

_C.DATA.GROUNDTRUTH = CN(new_allowed=True)

_C.DATA.GROUNDTRUTH.QUERY = CN()
_C.DATA.GROUNDTRUTH.QUERY.TEST = ""
_C.DATA.GROUNDTRUTH.QUERY.VALID = ""

_C.DATA.GROUNDTRUTH.CANDIDATE = CN()
_C.DATA.GROUNDTRUTH.CANDIDATE.TEST = ""
_C.DATA.GROUNDTRUTH.CANDIDATE.VALID = ""


_C.MODEL.MEMORYBANK = CN()
_C.MODEL.MEMORYBANK.QUEUE_MAX_SIZE = 1000
_C.MODEL.MEMORYBANK.N_BATCH = 50
_C.MODEL.MEMORYBANK.PROTO_BANK_SIZE = 54# sum(_C.DATA.ATTRIBUTES.NUM)
_C.MODEL.MEMORYBANK.MOMENTUM = 0.5


# SOLVER
_C.SOLVER = CN()
_C.SOLVER.EVAL_STEPS = 1
_C.SOLVER.OPTIMIZER_NAME = "Adam"
_C.SOLVER.STEP_SIZE = 3
_C.SOLVER.DECAY_RATE = 0.9
_C.SOLVER.EPOCHS = 80
_C.SOLVER.LOG_PERIOD = 400
_C.SOLVER.BASE_LR = 5e-5
_C.SOLVER.BASE_LR_SLOW = 1e-5

_C.SOLVER.INTER_WEIGHT = 0.04
_C.SOLVER.REGION_WEIGHT = 1.
_C.SOLVER.PATCH_WEIGHT = .1

_C.SOLVER.MARGIN = 0.2
_C.SOLVER.PROTO_MARGIN = 0.2
_C.SOLVER.BETA = 0.4
_C.SOLVER.TAU = 0.07
_C.SOLVER.ALPHA = 12.

# Logger
_C.LOGGER = CN()
_C.LOGGER.LEVEL = logging.INFO
_C.LOGGER.STREAM = "stdout"