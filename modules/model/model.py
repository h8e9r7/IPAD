...
class RPF(nn.Module):
    def __init__(self, cfg):
        ...
        self.vmlp = ValueMlp(cfg.DATA.ATTRIBUTES.NUM,
                             cfg.MODEL.EMBED_SIZE,
                             cfg.MODEL.ATTRIBUTE.EMBED_SIZE
                            )
        self.vembed = ValueEmbedding(cfg.DATA.ATTRIBUTES.NUM,
                                     cfg.MODEL.TRANSFORMER.EMBED_SIZE
                                    )
        self.vcrossencoder = ValueCrossEncoder(cfg.MODEL.EMBED_SIZE,
                                               cfg.MODEL.TRANSFORMER.EMBED_SIZE,
                                               cfg.MODEL.TRANSFORMER.REDUCTION_RATE,
                                               cfg.MODEL.TRANSFORMER.DROPOUT_RATE,
                                               cfg.MODEL.TRANSFORMER.NUM_HEADS,
    ...
    def value_forward(self, x, attmap, a):
        lx_v_out = self.vmlp(x, a)
        lx_v_pred = []
        for _v_out in lx_v_out:
            lx_v_pred.append(torch.argmax(_v_out, 0))
        lx_v_pred = torch.stack(lx_v_pred)
        lx_v_embed = self.vembed(a, lx_v_pred).unsqueeze(1)
        vx = self.vcrossencoder(attmap, lx_v_embed)
        return vx, lx_v_pred
    ...
    def forward(self, x, a, level='global'):
        ...
        if level=='global':
            ...
            x, attmap=self.choices["local"]['basenet'](x, a)
            return x, attmap
...
class Transformer(nn.Module):
    ...
    def forward(self, x, a):
        ...
        x_output, x_attmap=self.crossencoder(x_output, a)
...
class ClsEmbeddings(nn.Module):
    ...
    def forward(self, x, a):
        ...
        a = a.expand(-1, self.n_patches+1, -1)

        embeddings = x + self.position_embeddings # + a
...
class AttnEmbedding(nn.Module):
    def __init__(
        ...
        # AttnFashion
        self.fc_adapter = nn.Linear(final_embed_size, 1)
        self.softmax_adapter = nn.Softmax(dim=1)

        print('############ISLN [ON]')
        print('############Adapt Attentive Suppression [ON]')
    def forward(self, x, a):
        B,C,H,W = x.shape
        ###############################
        ...
        for i in range(N):
            p = a
            attmap, attmap_hat = self.spatial_attn(I, p)
            x_hat = I * attmap_hat            
            x = I * attmap
            ...
            # chanel attn
            m = self.channel_attn(x, p)
            m_hat = self.channel_attn(x_hat, p)
            p = x * m
            p_hat = x_hat * m_hat
        x = p
        x_hat = p_hat
        ...
        """
        Uncomment ada block and comment feature_fc
        to enable AttnFashion        
        """
        x = self.feature_fc(x)
        # did they forget?
        x_hat = self.feature_fc(x_hat)

        # # AttnFashion
        # ada_a = self.fc_adapter(x1)
        # ada_b = self.fc_adapter(x)
        # ada = torch.cat((ada_a, ada_b), dim=1)
        # ada = self.softmax_adapter(ada)
        # ada_a, ada_b = ada.split(1, dim=1)
        # x = ada_a * x1 + ada_b * x
        # x=self.feature_fc(x)
    ...
    def spatial_attn(self, x, a):	
        ...
        # 自适应注意力擦除
        attmap = self.adapt_attentive_suppress(attmap, threshold=0.95)
    ...
    def adapt_attentive_suppress(self, x, threshold=0.95):
        w = x.size(-1)
        h = x.size(-2)
        A = torch.sum(x, dim=1, keepdim=True)
        t = torch.max(A, dim=3, keepdim=True)[0]
        t = torch.max(t, dim=2, keepdim=True)[0]
        M = (A <= threshold * t).float()
        x = x * M
        return x
...
class ValueCrossEncoder(nn.Module):
    def __init__(self, final_embed_size, embed_size, reduction_rate, dropout_rate, num_heads, att_dropout_rate):
        super(ValueCrossEncoder, self).__init__()
        self.hidden_size = embed_size
        self.cross_attention_norm = LayerNorm(embed_size, eps=1e-6)
        self.cross_attn = CrossAttention(num_heads,embed_size,att_dropout_rate)
    ...
    def forward(self, x, v): 
        # x : batch * 196 * 768, feature
        # v : batch * 1 * 768, feature

        # Layer Norm
        x = self.cross_attention_norm(x)
        v = self.cross_attention_norm(v)
        # Cross Attention
        feature, _ = self.cross_attn(x, v)
        # Map the vector into 1024
        feature =self.fc(feature).squeeze()
        # MLP
        h = feature
...
class ValueMlp(nn.Module):
    def __init__(self,attr_nums,embed_size,mlp_dim,dropout_rate=0.1):
        ...
        self.fc1_layers = nn.ModuleList(
            [nn.Linear(embed_size, mlp_dim) for _ in self.attr_nums]
        )
        self.fc2_layers = nn.ModuleList(
            [nn.Linear(mlp_dim, embed_size) for _ in self.attr_nums]
        )
        
        self.output_layers = nn.ModuleList(
            [nn.Linear(embed_size, value) for value in self.attr_nums]
        )
    ...
    def _init_weights(self):
        ...
        for output_layer in self.output_layers:
            nn.init.normal_(output_layer.bias, std=1e-6)
        for l in self.fc1_layers:
            nn.init.xavier_uniform_(l.weight)
        for l in self.fc1_layers:
            nn.init.normal_(l.bias, std=1e-6)
        for l in self.fc2_layers:
            nn.init.xavier_uniform_(l.weight)
        for l in self.fc2_layers:
            nn.init.normal_(l.bias, std=1e-6)
    ...
    def forward(self, x, a):
        ...
        for i in range(x.size(0)):
            fc1 = self.fc1_layers[a[i]]
            fc2 = self.fc2_layers[a[i]]
            output_layer = self.output_layers[a[i]]

            x_i = self.dropout(nn.functional.relu(fc1(x[i].unsqueeze(0))))
            x_i = self.dropout(nn.functional.relu(fc2(x_i)))
            x_i = output_layer(x_i)

            output.append(x_i.squeeze(0))
        return output