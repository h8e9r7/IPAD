...
def build_optimizer(cfg, model):
    if not cfg.MODEL.TRANSFORMER.ENABLE or cfg.MODEL.SINGLE.ENABLE :
        ...
        parameters = [
            ...
            # add
            {'params': filter(lambda p: p.requires_grad, model.vmlp.parameters())},
            {'params': filter(lambda p: p.requires_grad, model.vembed.parameters())},
            {'params': filter(lambda p: p.requires_grad, model.vcrossencoder.parameters())},
        ]
